...
import org.robocup.gamecontroller.rules.HumanoidLeagueAdultSizeRuleBook;
...
public class GameController {
	...
	public GameController(String args[]) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			// look for turned on autopause
			if (args[i].equals(Constants.ARG_AUTOPAUSE)) {
				autopause = true;
				System.out.println("Autopause on");
				continue;
			}			
			...
			if (args[i].equals(Constants.ARG_HLADULT)) {
				leagueType = leagueHlAdult;
				continue;
			}
		...
		} else if (leagueType == leagueHlAdult) {
			rulebook = new HumanoidLeagueAdultSizeRuleBook();
		}

		if (configDir != null) {
			rulebook.setConfigDirectory(File.separator + configDir);
		}
		...
		// start the GUI
		MainGUI gui = new MainGUI(rulebook, teamNames, teamNumbers, broadcast, port, range, numPlayers, quiet, autopause);
