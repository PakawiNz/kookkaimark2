...
public class MainGUI extends javax.swing.JFrame {
	...
	public MainGUI(RuleBook rulebook, String[] teamNames, byte[] teamNumbers, String broadcastAddr, int port, int range, int numPlayers, boolean quiet, boolean autopause) {
	...
	protected JToggleButton buildButton(String label, String toolTip) {
		return buildButton(label, toolTip, null);
	}

	protected JToggleButton buildButton(String label, String toolTip, String label2) {
		JToggleButton button = new JToggleButton();
		button.setMargin(new Insets(2, 2, 2, 2));
		if (label2 != null) {
			label = "<html><center>" + label + "<br/>" + label2 + "</center></html>";
		}
	...
	protected JPanel initKickOffPanel() {
		...
		for (byte j = 0; j < Constants.NUM_TEAMS; j++) {
			cmdTeamKickOff[j] = buildButton(rulebook.getTeamColorName(j), "Click this button to select the " + rulebook.getTeamColorName(j) + " team for the next kick off.", "KickOff");
		...
		if (rulebook.getDropBall()) {
			cmdDropBall = buildButton("DropBall", "Click this button to select a drop ball.");
	...
	protected JPanel initGameStatePanel() {
		...
		gameStateButtons = new JToggleButton[5];
		gameStateButtons[0] = cmdInitial;
		gameStateButtons[1] = cmdReady;
		gameStateButtons[2] = cmdSet;
		gameStateButtons[3] = cmdKickOff;
		gameStateButtons[4] = cmdFinish;			
	...
	protected JPanel initGameDetailsPanel() {
		...
		pnlDetails.setLayout(new GridLayout(2, 1, 0, 0));
		...
		if(autopause)
			chkAutoPause.setSelected(true);

		chkStateSafe = new JCheckBox();
		chkStateSafe.setText("State Safe");
		chkStateSafe.setToolTipText("Check this option to allow state changes only in the correct order.");
		chkStateSafe.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				updateGameStateButton(data.getGameState());
			}
		});
		chkStateSafe.setSelected(true);
		pnlDetails.add(chkStateSafe);	
	...
	protected JPanel initPenaltyPanel() {
		...
		while (penalties.hasNext()) {
			...
			String penaltyDetails = null;
			if (rulebook.getPenaltyTimeShown()) {
				penaltyDetails = "(";
				if (rulebook.getPenaltyIsAddon(code)) {
					penaltyDetails += "+";
				}
				penaltyDetails += rulebook.getPenaltyTime(code);
				penaltyDetails += "s)";
			}
			JToggleButton button = buildButton(name, "Sanction a \"" + name + "\" penalty to the selected players.", penaltyDetails);
	...
	protected JPanel initTeamPanels() {
		...
		lblTeamPushCount = new JTextField[Constants.NUM_TEAMS];
		...
		for (byte j = 0; j < Constants.NUM_TEAMS; j++) {
			...
			for (byte i = 0; i < numPlayers; i++) {
				...
				cmdTeam[j][i] = new PlayerButton(rulebook, player, rulebook.getTeamColorName(j));
			...
			if (rulebook.getUsePushingCounter()) {
				...
				lblTeamPushCount[j] = new JTextField("0");
				lblTeamPushCount[j].setHorizontalAlignment(JTextField.CENTER);
				lblTeamPushCount[j].setFont(new Font("Dialog", 1, 50));
				lblTeamPushCount[j].setToolTipText(teamNames[j] + " push count.");
				lblTeamPushCount[j].setBackground(null);
				...
				lblTeamPushCount[j].addKeyListener(new PushingCounterAdapter(this, j));			
	...
	private void cmdTeamTimeOut_Selected(ActionEvent evt, byte team) {
		...
		if(data.getSecondaryGameState() != Constants.STATE2_PENALTYSHOOT) {
			...
			setGameStateData(Constants.STATE_INITIAL);
			cmdInitial.setSelected(true);			
		} else {
			setGameStateData(Constants.STATE_SET);
			cmdSet.setSelected(true);			
		}
		startClock = false;
	...
	private void cmbHalfActionPerformed(ActionEvent evt) {
		...
		if (isPenaltyShoot) {
			...
			byte kickoffTeam = shotA ? TEAM_1 : TEAM_2;
			...
			cmdTeamScoreDown[kickoffTeam].setEnabled(false);
	...
	private void switchLogosNumbersScoresNames() {
		...
		// reset lastSeenIndicator
		for (byte j = 0; j < Constants.NUM_TEAMS; j++){
			data.setLastSeenIndicator(j, false);
			for (int i = 0; i < numPlayers; i++)
				cmdTeam[j][i].setLastSeenIndicator(false); 			
		}
	...
	private void cmdTeamScoreDown_clicked(ActionEvent evt, byte team) {
		...
		if (data.getSecondaryGameState() == Constants.STATE2_PENALTYSHOOT) {
			cmdTeamScoreDown[team].setEnabled(false);
			cmdTeamScoreUp[team].setEnabled(true);
		}
		
		data.setScore(team, teamScore);
		lblTeamScore[team].setText("" + teamScore);
		logger.info(teamNames[team] + " (" + rulebook.getTeamColorName(team) + ") " + " loses a goal [" + teamNames[TEAM_1] + " (" + rulebook.getTeamColorName(TEAM_1) + ") " + data.getScore(TEAM_1) + " : " + data.getScore(TEAM_2) + " " + teamNames[TEAM_2] + "(" + rulebook.getTeamColorName(TEAM_2) + ")]");		
	}
	...
	private void cmdTeamScoreUp_clicked(ActionEvent evt, byte team) {
		...
		logger.info(teamNames[team] + " (" + rulebook.getTeamColorName(team) + ") " + " scrore [" + teamNames[TEAM_1] + "(" + rulebook.getTeamColorName(TEAM_1) + ") " + data.getScore(TEAM_1) + " : " + data.getScore(TEAM_2) + " " + teamNames[TEAM_2] + "(" + rulebook.getTeamColorName(TEAM_2) + ")]");
		...
		if (playMode == Constants.PLAY_MODE_FIRST_TEAM_PENALTY || playMode == Constants.PLAY_MODE_SECOND_TEAM_PENALTY) {
			...
			setGameStateData(Constants.STATE_FINISHED);
			cmdFinish.setSelected(true);
			cmdTeamScoreDown[team].setEnabled(true);
			cmdTeamScoreUp[team].setEnabled(false);
			
			setEventTitle("Ready");
			eventSecs = rulebook.getReadySeconds();
			special_event = SPECIAL_EVENT_READY;
		} else {
			setGameStateData(Constants.STATE_READY);
			...
			if(secs <= rulebook.getReadySeconds() && !chkAutoPause.isSelected()) {
	...
	private void applyPenalty(short team, short player, short code) {
		...
		if (rulebook.getUsePushingCounter() && data.getGameState() == Constants.STATE_PLAYING && code == Constants.PENALTY_SPL_PLAYER_PUSHING && rulebook.getPushingCounter((byte) team) == rulebook.getPushingThreshold(numEjected[team]) - 1) {
		...
		if (rulebook.getUsePushingCounter() && data.getGameState() == Constants.STATE_PLAYING && code == Constants.PENALTY_SPL_PLAYER_PUSHING) {
	...
	public void pushingCounterChanged(byte team) {
		int count = rulebook.getPushingCounter(team);
		int old = count;
		...
		// get new value
		try {
			count = Integer.parseInt(lblTeamPushCount[team].getText());
		} catch (NumberFormatException ex) { }
		  
		// check new value
		if(old != count) {
			lblTeamPushCount[team].setText(Integer.toString(count));
			rulebook.changePushingCounter(team, count);
			logger.info("Pushing counter for " + teamNames[team] + " (" + rulebook.getTeamColorName(team) + ") changed to " + rulebook.getPushingCounter(team));	    	  
		} 
		else lblTeamPushCount[team].setText(Integer.toString(old));
	}	
	...
	public void setGameState(byte state) {
		...
		} else if (state == Constants.STATE_SET) {
			if(oldGameState == Constants.STATE_FINISHED) {
				if (comboBoxModelHalf.getSelectedIndex() == Constants.PLAY_MODE_FIRST_TEAM_PENALTY) {
					cmbHalf.setSelectedIndex(comboBoxModelHalf.getIndexOfName(Constants.PLAY_MODE_SECOND_TEAM_PENALTY));
				} else if (comboBoxModelHalf.getSelectedIndex() == Constants.PLAY_MODE_SECOND_TEAM_PENALTY) {
					cmbHalf.setSelectedIndex(comboBoxModelHalf.getIndexOfName(Constants.PLAY_MODE_FIRST_TEAM_PENALTY));
				}
			}
	...
	public void setGameStateData(byte state) {
		broadcast.setBurst(true);
		data.setGameState(state);
		updateGameStateButton(state);			
	}	
	
	public void updateGameStateButton(byte state) {
		if(chkStateSafe.isSelected()) {
			byte[] succStates;
			byte sndGameState = data.getSecondaryGameState();

			cmdInitial.setEnabled(false);
			cmdReady.setEnabled(false);
			cmdSet.setEnabled(false);			
			cmdKickOff.setEnabled(false);
			cmdFinish.setEnabled(false);
			
			if(sndGameState != Constants.STATE2_PENALTYSHOOT) {
				if (state == Constants.STATE_INITIAL)
						succStates = Constants.SUCC_STATE_INITIAL;
				else if (state == Constants.STATE_READY)
					succStates = Constants.SUCC_STATE_READY;
				else if (state == Constants.STATE_SET)
					succStates = Constants.SUCC_STATE_SET;
				else if (state == Constants.STATE_PLAYING)
					succStates = Constants.SUCC_STATE_PLAYING;	
				else
					succStates = Constants.SUCC_STATE_FINISHED;
			} else {
				if (state == Constants.STATE_INITIAL)
					succStates = Constants.SUCC_STATE_PENALTY_INITIAL;				
				else if (state == Constants.STATE_READY)
					succStates = Constants.SUCC_STATE_PENALTY_READY;
				else if (state == Constants.STATE_SET)
					succStates = Constants.SUCC_STATE_PENALTY_SET;
				else if (state == Constants.STATE_PLAYING)
					succStates = Constants.SUCC_STATE_PENALTY_PLAYING;	
				else
					succStates = Constants.SUCC_STATE_PENALTY_FINISHED;				
			}
			
			gameStateButtons[state].setEnabled(true);
			
			for(int i = 0; i < succStates.length; i++) {
				gameStateButtons[succStates[i]].setEnabled(true);			
			}
		} else {
			cmdInitial.setEnabled(true);
			cmdReady.setEnabled(true);
			cmdSet.setEnabled(true);			
			cmdKickOff.setEnabled(true);
			cmdFinish.setEnabled(true);
		}
	}
	...
	public void updateRobot(RobotState state) {
		...
		switch (message) {
			...
			case Constants.GAMECONTROLLER_RETURN_MSG_ALIVE: // nothing to do, just a placeholder for alive message
				data.setLastSeenIndicator(team, true);
				for (int i = 0; i < numPlayers; i++) {
					cmdTeam[team][i].setLastSeenIndicator(true);
				}             
				break;
			default:
	...
	private JCheckBox chkStateSafe;	
	...
	JToggleButton[] gameStateButtons;
	...
	private JTextField[] lblTeamPushCount;
