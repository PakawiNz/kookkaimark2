...
public class MainGUI extends JFrame implements KeyListener {
	...
	public MainGUI(int port, boolean penaltyDisplay, int mode, GameState gameData, int display, boolean fullscreen, String configDir, boolean debug) {
		super("RoboCup GameStateVisualizer", GraphicsEnvironment.getLocalGraphicsEnvironment().getScreenDevices()[display].getDefaultConfiguration());
	...
	private void paintGameState(Graphics g) {
		...
		} else if (mode == Constants.MODE_HLKID || mode == Constants.MODE_HLTEEN || mode == Constants.MODE_HLADULT) {
		...
		for (byte i = 0; i <= 1; i++) {
			...
			if (timeWhenLastPacketReceived.getTime() > 0) {
				...
				if (i == 1)
					g.drawString(score, w / 2 - fm.stringWidth(score + " ") - fm.stringWidth(":") / 2, 1 * h / 2);
				else
					g.drawString(score, w / 2 + fm.stringWidth(":") / 2 + fm.stringWidth(" "), 1 * h / 2);
	...
	private String getLeague(int mode) {
		...
		else if (mode == Constants.MODE_HLADULT)
			return "config/hl-adult";
		else if (mode == Constants.MODE_SSL)
			return "config/ssl";
		return null;
	}
