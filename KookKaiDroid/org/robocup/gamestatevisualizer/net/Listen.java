...
public class Listen implements Runnable {
	...
	public Listen(int port, MainGUI gui) {
		...
		try {
			...
			System.err.println("GameStateVisualizer (constructor) error: " + err.getMessage() + " " + "(No network devices set up?)");
			System.exit(1);
		}
	}
	...
	public void run() {
		while (!stopController) {
			try {
				...
				if (gui.getDebug()) {
					System.out.println("Incomming packet");
				}
				...
				byte kickOffTeam = gameData.getKickOffTeam();
				short oldTeam2Score = gameData.getTeam(kickOffTeam).getTeamScore();

				// read package
				gameData.readPacket(packet, gui);

				if (oldSecondaryGameState == Constants.STATE2_NORMAL && gameData.getSecondaryGameState() == Constants.STATE2_PENALTYSHOOT) {
					for (byte i = 0; i <= 1; i++)
				...
				if (oldSecondaryGameState == Constants.STATE2_PENALTYSHOOT) {
					// miss
					if (oldHalf != gameData.getHalf()) {
						if (!penaltySuccess) {
							Vector<String> states = gui.getPenaltyStates().get("" + (int) gameData.getTeam(kickOffTeam).getTeamNumber());
							states.add("" + '\u25CB');
						} else
							penaltySuccess = false;
					}
					else if (oldHalf == gameData.getHalf()) {
						// decrease
						if (oldTeam2Score > gameData.getTeam(kickOffTeam).getTeamScore()) {
							Vector<String> states = gui.getPenaltyStates().get("" + (int) gameData.getTeam(kickOffTeam).getTeamNumber());
							int lastState = states.size() - 1;
							if (states.get(lastState).equals("" + '\u25CF')) // decrease only by goal
								states.remove(lastState);		
						}
						// goal
						else if (oldTeam2Score < gameData.getTeam(kickOffTeam).getTeamScore()) {
							Vector<String> states = gui.getPenaltyStates().get("" + (int) gameData.getTeam(kickOffTeam).getTeamNumber());
							states.add("" + '\u25CF');
							penaltySuccess = true;
						}
	...
	public void socketCleanup() {
		try {
			if (gui.getDebug()) {
				System.out.println("Stopping listen thread");
			}
			stopController = true;
			if (gui.getDebug()) {
