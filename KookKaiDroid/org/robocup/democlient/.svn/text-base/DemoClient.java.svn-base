...
package org.robocup.democlient;
...
public class DemoClient extends WindowAdapter {
		...
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				if (args.length < NUM_MIN_ARGS) {
					System.err.println("Usage: java -jar DemoClient teamNumber robotNumber\n");
					System.exit(1);
				}
				...
				byte teamId = Byte.parseByte(args[0]);
				byte robotId = Byte.parseByte(args[1]);

				RobotState state = new RobotState();
				state.setTeamId(teamId);
				state.setRobotId(robotId);
				System.out.println("Listen to team id " + teamId + " as robot number " + robotId);
	...
	public void update(GameState rgcd) {
		...
		if (found) {
			...
			// @FIXME the penalty numbers of the various leagues overlap
			// only a specific can be shown here
			switch (robotInfo.getPenalty()) {
				...
				case Constants.PENALTY_MANUAL:
					...
					break;
				default:
					penalty = "Penalty #" + robotInfo.getPenalty();
					break;
			}
			if (penalty != "") {
