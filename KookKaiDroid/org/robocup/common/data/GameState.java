...
public class GameState implements Provider {
	...
	// get/set for teams lastSeenIndicator
	public synchronized boolean getLastSeenIndicator(byte team) {
		return teams[team].getLastSeenIndicator();
	}

	public synchronized void setLastSeenIndicator(byte team, boolean state) {
		teams[team].setLastSeenIndicator(state);
	}	
	...
	public boolean readPacket(DatagramPacket packet, MainGUI gui) {
		...
		byte kickOffTeam = packetData.get(); // kickOffTeam
		...
		this.setKickOffTeam(kickOffTeam);
