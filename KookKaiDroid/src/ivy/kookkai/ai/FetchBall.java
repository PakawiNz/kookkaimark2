...
public class FetchBall implements AITemplate {
	...
	private double ballInnerDistance = 50;
	private double ballInnerDegree = 25;
	private double ballOuterDegree = 50;
	private double ballOuterDistance = 70;
	...
	public boolean enterOuterBallDistance() {
		return GlobalVar.ballPos[1] < ballOuterDistance &&  Math.abs(GlobalVar.ballPos[0] - 26) < ballOuterDegree;
	}
	
	public boolean enterInnerBallDistance() {
		return GlobalVar.ballPos[1] < ballInnerDistance && Math.abs(GlobalVar.ballPos[0] - 26) < ballInnerDegree;
	}
	...
	public String execute() {
		...
		String out = "";
		if(enterInnerBallDistance()) {
			driftState = true;
		}
		if(!enterOuterBallDistance()) {
			driftState = false;
		}
		String strategyOutput = strategy.run();
		out += "X = " + vx + "\n";
		out += "Y = " + vy + "\n";
		out += "Z = " + vz + "\n";
		out += "driftState : " + driftState + "\n";
		return out + strategyOutput;
	}
}
