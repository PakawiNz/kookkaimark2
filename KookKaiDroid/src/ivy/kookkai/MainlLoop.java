...
import ivy.kookkai.movement.PakawiNz_Movement;
...
public class MainlLoop implements Runnable {
	...
	public MainlLoop(CameraInterface cam,SensorInterface sens, DebugImgView dimg, TextView dtext, CheckBox drawColor) {
		...
		movement = new PakawiNz_Movement(robotAPI);
		strategy = new ChampStateFull(movement);
...
//		outString += (timeleft / 60) + ":" + (timeleft % 60) + "\n";
		...
		// NOTE: execute AI
		outString += sensorInterface.execute();
		outString += strategy.run();

		// NOTE: execute Control
		debugText.setText(outString);
		debugImg.invalidate(); // Important: must be last because it invalidates
								// the whole view
		...
		// else if(gameData.secondaryState==1)
		// debugText.append("\n"+"Extra:PenaltyShoot");
		// else if(gameData.secondaryState==2)
		// debugText.append("\n"+"Extra:Overtime");
		//
		// for(byte i=0;i< Constants.MAX_NUM_PLAYERS-8;i++){
		// if(KookKaiTeamInfo.getInstance().getTeamInfo().player[i].penalty==0)
		// debugText.append("\n"+"Player"+i+"Penalty:None");
		// else
		// if(KookKaiTeamInfo.getInstance().getTeamInfo().player[i].penalty==1)
		// debugText.append("\n"+"Player"+i+" Penalty:Ball Manipulation");
		// else
		// if(KookKaiTeamInfo.getInstance().getTeamInfo().player[i].penalty==2)
		// debugText.append("\n"+"Player"+i+" Penalty:Ball Physical Contact");
		// else
		// if(KookKaiTeamInfo.getInstance().getTeamInfo().player[i].penalty==3)
		// debugText.append("\n"+"Player"+i+" Penalty:Illegal Attack");
		// else
		// if(KookKaiTeamInfo.getInstance().getTeamInfo().player[i].penalty==4)
		// debugText.append("\n"+"Player"+i+" Penalty:Illegal Defence");
		// else
		// if(KookKaiTeamInfo.getInstance().getTeamInfo().player[i].penalty==5)
		// debugText.append("\n"+"Player"+i+" Penalty:Request for Pickup");
		// else
		// if(KookKaiTeamInfo.getInstance().getTeamInfo().player[i].penalty==6)
		// debugText.append("\n"+"Player"+i+" Penalty:Request for Service");
		// else
		// if(KookKaiTeamInfo.getInstance().getTeamInfo().player[i].penalty==7)
